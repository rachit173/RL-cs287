...
class ValueIteration(object):
    ...
    def get_next_values(self):
        ...
        if self.policy_type == 'deterministic':
            # V_{i+1}(s) = max _a \Sum_{s'} T(s,a,s') [R(s,a,s') + yV_i(s')]
            return np.max(
                np.sum(self.transitions * (self.rewards + self.discount * self.value_fun.get_values())
                , axis=2)
                , axis=1)
            # Code here for part 1

        elif self.policy_type == 'max_ent':
            # V_{i+1}(s) = max _a \Sum_{s'} T(s,a,s') [R(s,a,s') + yV_i(s')]
            Q = np.sum(self.transitions * (self.rewards + self.discount * self.value_fun.get_values()), axis=2)
            Q = (Q.T - np.max(Q, axis=1)).T + self.eps
            V = (1.0 / self.temperature) * np.log(
                np.sum(np.exp(self.temperature * Q), axis=1))
    ...
    def get_next_policy(self):
        ...
        """INSERT YOUR CODE HERE"""
        if self.policy_type == 'deterministic':
            # print(f'Transitions dimensions: {self.transitions.shape}')
            # print(f'Reward dimensions:  {self.rewards.shape}')
            ...
            actions = np.argmax(
                ...
                , axis=2)
                , axis=1)
            policy[np.arange(num_states), actions] = 1.0
            return policy
        elif self.policy_type == 'max_ent':
            # V_{i+1}(s) = max _a \Sum_{s'} T(s,a,s') [R(s,a,s') + yV_i(s')]
            Q = np.sum(self.transitions * (self.rewards + self.discount * self.value_fun.get_values()), axis=2)
            Q = (Q.T - np.max(Q, axis=1)) + self.eps
            R = (np.exp(self.temperature * Q) / np.sum(np.exp(self.temperature * Q), axis=0)).T
